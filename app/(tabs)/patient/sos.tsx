          ...
          : events.map(ev => (
              <Card key={ev._id} style={s.histCard}>
                <Row style={{ gap: 10 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={s.histDate}>{new Date(ev.createdAt).toLocaleString()}</Text>
                    {ev.message && <Text style={s.histMsg}>{ev.message}</Text>}
                    {ev.location && (
                      <TouchableOpacity onPress={() => Linking.openURL(`https://maps.google.com/?q=${ev.location!.lat},${ev.location!.lng}`)}>
      ...
      <Modal visible={showConfirm} transparent animationType="fade">
        <View style={s.overlay}>
          <Card style={s.confirmCard}>
            ...
            <Input
              label="Family contact number (optional, for SMS)"
...
const s = StyleSheet.create({
  ...
  sosBtn:      {
    backgroundColor: Colors.danger, borderRadius: Radius.xl, padding: Spacing.xl,
    alignItems: "center", gap: 4,
    shadowColor: Colors.danger, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 16, elevation: 8,
  },
