...
export default function QRScan() {
  ...
  const handleScan = async ({ data }: { data: string }) => {
    ...
    setLoading(true);
    try {
      // The QR encodes raw JSON: { patientId, code }. The backend's
      // /auth/qr-login reads `req.body.qrData` and does JSON.parse(qrData)
      // itself — so we must forward the RAW scanned string under the key
      // `qrData`, not repackage/rename it. Previously this sent `qrToken`,
      // which the backend never read, so login always silently failed.
      let parsed: any;
      try { parsed = JSON.parse(data); } catch { parsed = null; }
      if (!parsed || !parsed.patientId) {
        throw new Error("This doesn't look like a NurseCare+ QR code.");
      }

      const { token, user } = await authApi.qrLogin({ qrData: data });
      await login(token, user);
      router.replace("/(tabs)/home");
    } catch (e: any) {
      Alert.alert("QR Login Failed", e.message ?? "Please try again.", [
...
const s = StyleSheet.create({
  ...
  finder: { width: FINDER, height: FINDER, backgroundColor: "transparent" },
